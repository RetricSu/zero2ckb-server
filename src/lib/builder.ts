...
export class Builder {
  ...
  sign_Multisig(
    ...
    for (let i = 0; i < input_groups.length; i++) {
      ...
      const serializedMultisigScript =
        this.serializeMultisigScript(multisigScript);
      ...
      for (let l = 0; l < account_ids.length; l++) {
        ...
        witness_args.lock =
          witness_args.lock?.substring(0, sig_offset) +
          signature.slice(2) +
          witness_args.lock?.substring(sig_offset + 130);
      }
  ...
  deploy_contract(
    ...
    if (mode === "upgradable")
      return this.deploy_upgradable_contract(
        compiled_code,
        length,
        raw_tx_without_output,
        input_cells,
        account_id,
        tx_fee
      );
    ...
    const remain_balance =
      BigInt(input_cells[0].capacity) -
      BigInt(raw_tx_without_output.outputs[0].capacity) -
      BigInt(tx_fee) * 100000000n;
    if (remain_balance > 0) {
  ...
  deploy_upgradable_contract(
    ...
    const first_input_outpoint: OutPoint =
      raw_tx_without_output.inputs[0].previous_output;
    ...
    console.log(
      this.generateTypeIDArgsHash(first_input_outpoint, first_output_index)
    );
    raw_tx_without_output.outputs_data[0] = compiled_code;
  ...
  generateScriptHash(script: Script) {
    const serialized_script = core.SerializeScript(
  ...
  generateTypeIDArgsHash(
    ...
    const f_op = core.SerializeOutPoint(
      normalizers.NormalizeOutPoint(first_input_outpoint)
    );
    const hasher = new CKBHasher();
    hasher.update(f_op);
  ...
  generateTypeIDArgs(first_input: Input): HexString {
    return (
      "0x" +
      buf2hex(
        core.SerializeCellInput(normalizers.NormalizeCellInput(first_input))
      )
    );
  }
  ...
  generateMultisigArgs(
    serializedMultisigScript: string,
    since: string
  ): string {
    let sinceLE = "0x";
    if (since != null) {
    ...
    return (
      new CKBHasher()
        ...
        .digestHex()
        .slice(0, 42) + sinceLE.slice(2)
    );
  }

  serializeMultisigScript(multisigScript: MultisigScript): string {
    if (multisigScript.S !== 0)
      throw new Error("S in MultisigScript must be 0 now!");
  ...
  toMessage(
    ...
    for (let i = 0; i < input_groups.length; i++) {
      ...
      for (let j = 1; j < input_groups[i].child.length; j++) {
        const witness_index = input_groups[i].child[j];
        const witness_in_group = new Reader(
          witnesses[witness_index] ? witnesses[witness_index] : "0x"
        );
        this.hasher.update(serializeBigInt(witness_in_group.length()));
        this.hasher.update(witness_in_group);
      }
