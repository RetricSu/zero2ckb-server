...
inoremap <silent> <SNR>48_AutoPairsReturn =AutoPairsReturn()
cnoremap <C-N> <Down>
cnoremap <C-P> <Up>
cnoremap <C-K> 
cnoremap <C-E> <End>
...
nmap ys <Plug>Ysurround
nnoremap <SNR>204_: :=v:count ? v:count : ''
...
nmap <F1> :below terminal ++rows=10
...
set ignorecase
set incsearch
set indentkeys=0{,0},0),0],:,0#,!^F,o,O,e,0],0),*;,*<:>,*<Return>
set isfname=@,48-57,/,.,-,_,+,,,#,$,%,~,=,:
set langmenu=en
set laststatus=2
set lazyredraw
...
set runtimepath+=~/.vim/bundle/vim-styled-components
set runtimepath+=~/.vim/bundle/vim-graphql
...
set runtimepath+=~/.vim_runtime/sources_non_forked/nginx-vim-syntax
...
set runtimepath+=~/.vim_runtime/sources_non_forked/snipmate-snippets
set runtimepath+=~/.vim_runtime/sources_non_forked/syntastic
set runtimepath+=~/.vim_runtime/sources_non_forked/tabular
set runtimepath+=~/.vim_runtime/sources_non_forked/tlib
set runtimepath+=~/.vim_runtime/sources_non_forked/tpope-vim-abolish
...
set runtimepath+=~/.vim_runtime/sources_non_forked/vim-go
...
set runtimepath+=~/.vim/bundle/vim-jsx-typescript/after
set runtimepath+=~/.vim/bundle/vim-styled-components/after
set runtimepath+=~/.vim/bundle/vim-graphql/after
...
argglobal
%argdel
$argadd src/lib/builder.ts
edit src/lib/builder.ts
...
exe '2resize ' . ((&lines * 48 + 31) / 63)
exe 'vert 2resize ' . ((&columns * 78 + 136) / 272)
exe '3resize ' . ((&lines * 48 + 31) / 63)
exe 'vert 3resize ' . ((&columns * 78 + 136) / 272)
exe '4resize ' . ((&lines * 48 + 31) / 63)
exe 'vert 4resize ' . ((&columns * 78 + 136) / 272)
exe '5resize ' . ((&lines * 10 + 31) / 63)
exe 'vert 5resize ' . ((&columns * 236 + 136) / 272)
argglobal
enew
file NERD_tree_1
let s:cpo_save=&cpo
...
setlocal indentexpr=
setlocal indentkeys=0{,0},0),0],:,0#,!^F,o,O,e,0],0),*;,*<:>,*<Return>
...
setlocal spelloptions=
setlocal statusline=%{lightline#link()}%#LightlineLeft_inactive_0#%(\ %t\ %)%#LightlineLeft_inactive_0_1#\ %#LightlineMiddle_inactive#%=%#LightlineRight_inactive_1_2#\ %#LightlineRight_inactive_1#%(\ %3p%%\ %)%#LightlineRight_inactive_0_1#\ %#LightlineRight_inactive_0#%(\ %3l:%-2v\ %)
setlocal suffixesadd=.ts,.tsx,.js
setlocal noswapfile
setlocal synmaxcol=3000
...
setlocal wrapmargin=0
silent! normal! zE
let s:l = 415 - ((31 * winheight(0) + 24) / 48)
...
wincmd w
argglobal
if bufexists("src/test/server.ts") | buffer src/test/server.ts | else | edit src/test/server.ts | endif
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
inoremap <buffer> <silent> <expr> <M-p> AutoPairsToggle()
...
wincmd w
argglobal
if bufexists("src/lib/chain.ts") | buffer src/lib/chain.ts | else | edit src/lib/chain.ts | endif
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
inoremap <buffer> <silent> <expr> <M-p> AutoPairsToggle()
...
wincmd w
argglobal
terminal ++curwin ++cols=236 ++rows=10 
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
inoremap <buffer> <silent> <expr> <M-p> AutoPairsToggle()
...
exe 'vert 5resize ' . ((&columns * 236 + 136) / 272)
tabnext 1
badd +415 src/lib/builder.ts
badd +0 src/lib/chain.ts
badd +0 src/test/server.ts
